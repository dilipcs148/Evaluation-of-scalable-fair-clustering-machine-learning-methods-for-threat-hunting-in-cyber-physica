...
def tree_fairlet_decomposition(p, q, root, dataset, colors):
    ...
    #print("p is:",p)
    #print("q is:",q)
    #print("lenrootred is:",len(root.reds))
    #print("lenrootblue is:",len(root.blues))
...
if len(sys.argv) > 4:
    ...
    #data = readtable(input_csv_filename,'ReadVariableNames',false);
   # labels = data[:,1]; #added by dilip
    colors = []
    points = []
    i = 0
    skipped_lines = 0
...
print(len(FAIRLET_CENTERS))
fairlet_center_idx = [dataset[index] for index in FAIRLET_CENTERS]

#print("fairlet_center_idx",fairlet_center_idx)
#print(len(fairlet_center_idx))

#label= [dataset[index] for index in FAIRLET_CENTERS]

#print("fairlet_center_idx",fairlet_center_idx)


fairlet_center_pt = np.array([np.array(xi) for xi in fairlet_center_idx])
#print("fairlet_center_pt")
#print(fairlet_center_pt)

#put label col value i.e dimension-1
res = [sub[664] for sub in fairlet_center_pt]
print("Label",res)
print(len(res))
res1=np.asarray(res)
res1=res1.astype('int32')
res1=np.where(res1==0.0, 0, res1)
res1=np.where(res1==1.0, 1, res1)
#print("Label1",res1)
#print(len(res1))

fairlet_center_pt= np.delete(fairlet_center_pt, -1, axis=1)
#print("remove_fairlet_center_pt")
#print(fairlet_center_pt)
...
#print("mat_matx",mat_matrix)
#print(len(mat_matrix))

#predLabels = labels; # Initialize a vector for predicted labels.
#eng.predLabels(idx==1) = eng.categorical({'1'}); # Assign group 1 to be benign.
#eng.predLabels(idx==2) =  eng.categorical({'0'}); # Assign group 2 to be malware.
...
print("no of cluster is:",k)
for i in range(k): # (len(idx_val)):
 normalcount=0
 malwarecount=0
 for j in range(len(res_idx1)):    
  idx_val=i+1
  if res_idx1[j]==idx_val and res1[j]==1:
      #print("i is: ",i)
      #print("idx_val[i] is: ",idx_val[i])
      malwarecount=malwarecount+1
  if res_idx1[j]==idx_val and res1[j]==0:
    normalcount=normalcount+1
 print("idx:",idx_val,"normalcount",normalcount)
 print("idx:",idx_val,"malwarecount",malwarecount)
 if normalcount > malwarecount:
     res_idx1=np.where(res_idx1==idx_val, 0, res_idx1)
 else:
     res_idx1=np.where(res_idx1==idx_val, 1, res_idx1)


#res_idx1=np.where(res_idx1==1, 0, res_idx1)
#res_idx1=np.where(res_idx1==2, 0, res_idx1)
#res_idx1=np.where(res_idx1==3, 1, res_idx1)
#res_idx1=np.where(res_idx1==4, 1, res_idx1)
#print("res_idx1",res_idx1)
